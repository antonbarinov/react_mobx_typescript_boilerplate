            ...
            if (isEnumerable(newState)) {
                ...
                if (isSrcArray) {
                    newState.forEach((value, i) => {
                        ...
                        oldState[i] = mutateObjectFn(oldState[i], value, false);
                    });
                } else {
                    for (const i in newState) {
                        if (newState.hasOwnProperty(i)) {
                            ...
                            set(oldState, i, mutateObjectFn(oldState[i], newState[i], false));
                        }
                    }
                }
                return oldState;
        ...
        mutateObjectFn = (oldState, newState, preserveRootProps) => {
            if (isEnumerable(newState)) {
                ...
                if (isSrcArray) {
                    ...
                    for (const i in newState) {
                        if (newState.hasOwnProperty(i)) {
                            oldState[i] = mutateObjectFn(oldState[i], newState[i], false);
                        }
                    }
                }
                return oldState;
